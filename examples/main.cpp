...
int main(int, char**) {
  ...
  for(int i = 0; i < num_keys; i++) {
    std::cout << "values[" << i << "] = (" << values[i].first << ", " << values[i].second << ")" << std::endl;
}

  // Bulk load the keys [0, 100)
  alex_index.bulk_load(values, num_keys);

  // Insert the keys [100, 200). Now there are 200 keys.
  for (int i = num_keys; i < 2 * num_keys; i++) {
    KEY_TYPE new_key = i;
    PAYLOAD_TYPE new_payload = dis(gen);
    alex_index.insert(new_key, new_payload);
  }
  int test =42;
std::cout << alex_index.find(&test).payload() << std::endl;
  // Erase the keys [0, 10). Now there are 190 keys.
  for (int i = 0; i < 10; i++) {
    alex_index.erase(i);
  }

  // Iterate through all entries in the alex_index and update their payload if the
  // key is even
  int num_entries = 0;
  for (auto it = alex_index.begin(); it != alex_index.end(); it++) {
  ...
  if (num_entries != 190) {
    std::cout << "Error! There should be 190 entries in the alex_index."
              << std::endl;
  }
  ...
  // (exclusive)
  num_entries = 0;
  for (auto it = alex_index.lower_bound(50); it != alex_index.lower_bound(100); it++) {
    num_entries++;
  }
  if (num_entries != 50) {
    std::cout
  ...
  num_entries = 0;
  auto it = alex_index.lower_bound(50);
  while (it.key() < 100 && it != alex_index.end()) {
    num_entries++;
    it++;
  }
  if (num_entries != 50) {
  ...
  for (auto it = alex_index.lower_bound(42); it != alex_index.upper_bound(42); it++) {
    num_duplicates++;
  }
